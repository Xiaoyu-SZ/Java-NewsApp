...
public class ChannelAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ChannelAdapter(Context mContext, List<ChannelBean> mList, List<ChannelBean> recommendList, List<ChannelBean> cityList) {
        ...
        for(int i=0;i<mList.size();i++){
            ChannelBean cb=mList.get(i);
            if(cb.getName()=="新闻"){hasNews=true;}
            else if(cb.getName()=="论文"){hasPaper=true;}
        }
    }
        ...
        holder.name.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (holder.getLayoutPosition() < selectedSize + 1) {
                    ...
                    if(text=="新闻") {hasNews=false;}
                    else if(text=="论文"){ hasPaper=false;}
                    ...
                    selectedSize++;
                    String text=((TextView)v).getText().toString();
                    if(text=="新闻") {hasNews=true;}
                    else if(text=="论文") {hasPaper=true;}
        ...
        if (mLeft == -1) {
            holder.recommend.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
                ...
                public boolean onPreDraw() {
                    ...
                    mLeft = (int) (holder.recommend.getLeft() + layout.getPrimaryHorizontal(0)) - TActivity.dip2px(mContext, 10);   //textView左边距离+第一个文字绘制的距离 设置了padding 所以要减掉
        ...
        holder.city.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (isRecommend) {
                    ...
                    if (mRight == -1) {
                        mRight = mLeft + holder.city.getLeft() - TActivity.dip2px(mContext, 10);
                    }
